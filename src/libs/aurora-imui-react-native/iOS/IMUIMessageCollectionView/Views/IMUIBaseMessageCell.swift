...
open class IMUIBaseMessageCell: UICollectionViewCell, IMUIMessageCellProtocol {
  ...
  override init(frame: CGRect) {
    ...
    let avatarLongGes = UILongPressGestureRecognizer(target: self, action: #selector(self.longTapHeaderImage))
    avatarImage.addGestureRecognizer(avatarLongGes)
  ...
  func setupData(with message: IMUIMessageModelProtocol) {
    ...
    let avatar = message.fromUser.Avatar();
  ...
  @objc func tapHeaderImage() {
    ...
    @objc func longTapHeaderImage(sender: UITapGestureRecognizer){
        if (sender.state == .began) {
            self.delegate?.messageCollectionView?(didLongTapHeaderImageInCell: self, model: self.message!)
        }
    }
