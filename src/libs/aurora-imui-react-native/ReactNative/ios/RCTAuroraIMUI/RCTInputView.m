...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (self) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
      ...
      [[NSNotificationCenter defaultCenter] addObserver:self
                                               selector:@selector(addTextWithInputView:)
                                                   name:kAddTextWithInputView object:nil];
...
- (void)addTextWithInputView:(NSNotification *)noti{
    
    NSString *text = noti.object;
//    [self.imuiIntputView insertText:text];
    dispatch_sync(dispatch_get_main_queue(), ^{
        [self.imuiIntputView addTextToTextViewWithStr:text];
    });
