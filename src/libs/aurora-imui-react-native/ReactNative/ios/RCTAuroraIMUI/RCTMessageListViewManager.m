...
- (void)messageCollectionViewWithDidLongTapHeaderImageInCell:(UICollectionViewCell * _Nonnull)didLongTapHeaderImageInCell model:(id <IMUIMessageModelProtocol> _Nonnull)model;
...
RCT_EXPORT_VIEW_PROPERTY(onAvatarLongClick, RCTBubblingEventBlock)
...
- (void)messageCollectionViewWithDidLongTapHeaderImageInCell:(UICollectionViewCell *)didLongTapHeaderImageInCell model:(id<IMUIMessageModelProtocol>)model{
    if (!_messageList.onAvatarLongClick) {
        return;
    }
    RCTMessageModel *message = model;
    NSDictionary *messageDic = message.messageDictionary;
    _messageList.onAvatarLongClick(@{@"message": messageDic});
}
