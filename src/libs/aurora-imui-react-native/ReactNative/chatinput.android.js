...
export default class ChatInput extends Component {
	...
	_onTextChange(event: Event) {
		if (!this.props.onTextChange) {
			return;
		}
		this.props.onTextChange(event.nativeEvent);
	}
	...
	addText(text){
		UIManager.dispatchViewManagerCommand(findNodeHandle(this.refs[CHAT_INPUT]),103,[text]);
	}

	keyboardDismiss(){
		UIManager.dispatchViewManagerCommand(findNodeHandle(this.refs[CHAT_INPUT]),104,null);
	}
