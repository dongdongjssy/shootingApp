...
public class TxtViewHolder<MESSAGE extends IMessage> extends BaseMessageViewHolder<MESSAGE>
    ...
    public void onBind(final MESSAGE message) {
        ...
        if (!message.isOutgoing() && (message.getAtAll() || message.getAtMe())){

            String atText = "";
            String textStr = message.getText();
            if (message.getAtAll()){
                atText = "@所有人";
            }else if (message.getAtMe()){
                String selfNikeName = message.getSelfNikeName();
                String selfUserName = message.getSelfName();
                if (textStr.contains("@" + selfNikeName)){
                    atText = "@" + selfNikeName;
                }else {
                    atText = "@" + selfUserName;
                }
            ...
            int bstart = textStr.indexOf(atText);
            if (bstart == -1){
                mMsgTv.setText(textStr);
                return;
            }
            int bend = bstart +atText.length();
            SpannableStringBuilder style=new SpannableStringBuilder(textStr);
            style.setSpan(new BackgroundColorSpan(Color.parseColor("#a1a1a1")),bstart,bend, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        mAvatarIv.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                if (mAvatarLongClickListener != null){
                    mAvatarLongClickListener.onAvatarLongClick(message);
                }
                return true;
            }
        });
