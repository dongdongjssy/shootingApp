...
public class RCTMessageDeserializer implements JsonDeserializer<RCTMessage> {
    ...
    public RCTMessage deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext)
        ...
        String selfName = jsonObject.get("selfName").getAsString();
        String selfNikeName = jsonObject.get("selfNikeName").getAsString();
        boolean atMe =  jsonObject.get("atMe").getAsBoolean();
        boolean atAll =  jsonObject.get("atAll").getAsBoolean();
        boolean isOutgoing = jsonObject.get("isOutgoing").getAsBoolean();
        RCTMessage rctMessage = new RCTMessage(msgId, status, msgType, isOutgoing,selfName,atMe,atAll,selfNikeName);
