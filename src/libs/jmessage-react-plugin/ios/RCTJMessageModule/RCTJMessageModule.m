    ...
    [JMSGConversation allConversations:^(id resultObject, NSError *error) {
        ...
        if (conversationList.count < 1) {
            ...
            for (JMSGConversation *conversation in conversationList) {
                NSMutableDictionary *conDic = [conversation conversationToDictionary];
                if (conversation.conversationType == kJMSGConversationTypeGroup && conversation.unreadCount.intValue > 0 ) {
                    NSString * atMsgId = [self checkGroupAtMessage:conversation];
                    [conDic setValue:atMsgId forKey:@"atMsgId"];
                }else{
                    
                    [conDic setValue:@"-1" forKey:@"atMsgId"];
                }
                [conversationDicList addObject:conDic];
            }
...
- (NSString *)checkGroupAtMessage:(JMSGConversation *)conversation{
    
    NSArray *messages = [conversation messageArrayFromNewestWithOffset:0 limit:conversation.unreadCount];
    NSString *messageId = @"-1";
    for (NSInteger i = messages.count - 1;i >= 0;i--){
        JMSGMessage *msg = messages[i];
...
RCT_EXPORT_METHOD(createSendMessage:(NSDictionary *)param
    ...
    if ([param[@"messageType"] isEqualToString:@"image"] ||
        ...
        if (@available(iOS 8.0, *)) {
            if ([mediaPath containsString:@"ph://"]) {
                NSString *assestInden = [mediaPath stringByReplacingOccurrencesOfString:@"ph://" withString:@""];
                PHAsset *asset = [PHAsset fetchAssetsWithLocalIdentifiers:@[assestInden] options:nil].lastObject;
                PHAssetResource * resource = [[PHAssetResource assetResourcesForAsset: asset] firstObject];
                mediaPath = [[[resource valueForKey:@"privateFileURL"] absoluteString] stringByReplacingOccurrencesOfString:@"file://" withString:@""];
