...
const receiveApplyJoinGroupApprovalEvent =
  'JMessage.ReceiveApplyJoinGroupApprovalEvent'; // 接收到入群申请
const receiveGroupAdminRejectEvent = 'JMessage.ReceiveGroupAdminRejectEvent'; // 接收到管理员拒绝入群申请
const receiveGroupAdminApprovalEvent =
  'JMessage.ReceiveGroupAdminApprovalEvent'; // 接收到管理员同意入群申请
...
export default class JMessage {
   ...
   * 设置是否屏蔽群消息。
   *
   * @param {Object} params = { id: String, isBlocked: boolean }
   */
  ...
  static addReceiveMessageListener(listener) {
    listeners[listener] = DeviceEventEmitter.addListener(
      receiveMsgEvent,
      message => {
        listener(message);
      },
  ...
  static addReceiveApplyJoinGroupApprovalListener(listener) {
    listeners[listener] = DeviceEventEmitter.addListener(
      receiveApplyJoinGroupApprovalEvent,
      event => {
        listener(event);
      },
    );
  ...
  static addReceiveGroupAdminApprovalListener(listener) {
    listeners[listener] = DeviceEventEmitter.addListener(
      receiveGroupAdminApprovalEvent,
      event => {
        listener(event);
      },
    );
