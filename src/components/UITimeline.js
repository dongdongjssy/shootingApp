...
const defaultCircleColor = "#005CB6";
const defaultLineWidth = 2;
const defaultLineColor = "#005CB6";
const defaultTimeTextColor = "#8C9091";
...
export default class Timeline extends Component {
  ...
  _renderTime(rowData, sectionID, rowID) {
    ...
    // console.warn("timeWrapper",timeWrapper);
    return (
      <View style={timeWrapper}>
        <View style={[styles.timeContainer, this.props.timeContainerStyle]}>
          <Text numberOfLines={1} style={[styles.time, this.props.timeStyle]}>
            {rowData.time}
          </Text>
        </View>
      </View>
  ...
  _renderDetail(rowData, sectionID, rowID) {
    let title = rowData.description ? (
      ...
      <Text style={[styles.title, this.props.titleStyle,rowData.titleStyle]}>{rowData.title}</Text>
    );
    ...
    switch (this.props.columnFormat) {
      case "single-column-left":
        circleStyle = {
          ...
          left: this.state.x - circleSize / 2 + (lineWidth - 1) / 2+2
        };
        break;
      case "single-column-right":
        circleStyle = {
          ...
          borderRadius: circleSize / 2,
          backgroundColor: circleColor,
          left: this.state.width - circleSize / 2 - (lineWidth - 1) / 2+2
        };
        break;
      case "two-column":
        circleStyle = {
...
const styles = StyleSheet.create({
  ...
  detail: { paddingTop: 0, paddingBottom: 35 },
  description: {
    marginTop: 10
  },
  separator: {
